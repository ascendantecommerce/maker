      ...
      <Card className="border-2 shadow-2xl bg-card/40 backdrop-blur-md overflow-hidden">
        ...
        <CardContent className="space-y-6">
          <div className="space-y-3">
            <label className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Video Source URL</label>
            <Input 
              value={url} 
          ...
          <div className="space-y-3">
            <label className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Internal Name</label>
            <Input 
              value={name} 
          ...
          {triggered && (
            <div className="p-5 rounded-xl bg-green-500/10 border border-green-500/20 text-green-100 text-md animate-in fade-in zoom-in-95 duration-500">
              ...
              <p className="opacity-90">The event has been dispatched to Inngest. Monitor your terminal output to see each step returning its output JSON.</p>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="bg-card/20 border-primary/10 shadow-inner">
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-black uppercase text-muted-foreground">System Status</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-center gap-3">
              <div className={`h-4 w-4 rounded-full ${triggered ? 'bg-green-500 shadow-[0_0_12px_rgba(34,197,94,0.6)] animate-pulse' : 'bg-yellow-500 shadow-[0_0_8px_rgba(234,179,8,0.4)]'}`} />
              <span className="text-lg font-medium">{triggered ? "Live Task Running" : "Ready for Input"}</span>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-card/20 border-primary/10 shadow-inner">
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-black uppercase text-muted-foreground">Review Mode</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-sm font-medium text-muted-foreground leading-relaxed">
              Database persistence is currently <span className="text-white font-bold">DISABLED</span>. Review JSON results in logs.
            </p>
          </CardContent>
        </Card>
      </div>
