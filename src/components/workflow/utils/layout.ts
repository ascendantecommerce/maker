...
export const getLayoutedElements = (
  ...
  nodes.filter(n => n.type === 'shotGroup').forEach(sg => {
    ...
    const w = SHOT_PAD.left + (prompt ? nodeW(prompt) : 0) + (output ? INTERNAL_GAP + nodeW(output) : 0) + SHOT_PAD.right;
    const h = SHOT_PAD.top + Math.max(prompt ? nodeH(prompt) : 0, output ? nodeH(output) : 0) + SHOT_PAD.bottom;
    
    shotGroupSizes.set(sg.id, { w, h });

    // Position children of ShotGroup
    if (prompt) {
      rects.set(prompt.id, { x: SHOT_PAD.left, y: SHOT_PAD.top, w: nodeW(prompt), h: nodeH(prompt) });
    }
    if (output) {
      rects.set(output.id, { x: SHOT_PAD.left + (prompt ? nodeW(prompt) + INTERNAL_GAP : 0), y: SHOT_PAD.top, w: nodeW(output), h: nodeH(output) });
    }
  });
  ...
  nodes.filter(n => n.type === 'segmentContentGroup').forEach(cg => {
  ...
  nodes.filter(n => n.type === 'segmentGroup').forEach(segG => {
    ...
    const totalW = GROUP_PAD.left + cgSize.w + (visGSize.w > 0 ? INTERNAL_GAP + visGSize.w : 0) + GROUP_PAD.right;
  ...
  rects.set(globalOutput.id, { x: xOutput + 260 + COL_GAP, y: (totalH - nodeH(globalOutput)) / 2, w: nodeW(globalOutput), h: nodeH(globalOutput) });
