...
export class WanProvider implements Generator {
  ...
  async create(params: VideoParams): Promise<string> {
    const path = "image-to-video/wan-v2-2-720p";
    const body = {
      image: params.firstFrameUrl,
      prompt: params.prompt?.trim() || "",
      duration: params.durationSeconds?.toString() || "5",
    };
    return this.createTask(path, body, "Wan");
  }
