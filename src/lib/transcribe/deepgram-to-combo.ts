import { Paragraph, TranscriptObject, Word } from "./types";
import { detectLanguage } from "./detect-language";

const getWords = (deepgramResult: any): Word[] => {
  const words = deepgramResult.results.channels[0].alternatives[0].words.map((w: any) => {
    return {
...
export async function deepgramToCombo(deepgramResult: any): Promise<Partial<TranscriptObject>> {
