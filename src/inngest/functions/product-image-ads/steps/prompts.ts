import * as productServices from "../services/prompts";
import { VideoSchema } from "@/inngest/utils/types";
import { VisualShot, VisualBroll } from "@/lib/schema-generator/types";

export const generateProductPrompts = async (
  scheme: VideoSchema,
  generatedSchema: any,
  step: any,
) => {
  return step.run("generate-product-prompts-service", async () => {
    return productServices.generateProductPrompts(scheme, generatedSchema);
  });
};
