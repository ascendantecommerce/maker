...
export const generateCharacterSeedImages = async (
  ...
  const segmentShotUpdates: { segmentId: string; shotIndex: number; imageUrl: string }[] = [];
  ...
  for (const segment of scheme.segments) {
    ...
    for (let shotIndex = 0; shotIndex < shots.length; shotIndex++) {
      ...
      const prompt = shot.firstFramePrompt
        || `${globalStyle}, ${character.visualDescription?.trim() || character.role}, cinematic lighting, Pixar style`;

      console.log(`[Stage 1] Generating scene image for ${segment.id} shot ${shotIndex} (${shot.type}): ${prompt.slice(0, 80)}...`);
      ...
      const productImageUrls = shot.type === "product" && scheme.assets?.length
      ...
      segmentShotUpdates.push({ segmentId: segment.id, shotIndex, imageUrl: persistentUrl });
    }
  }

  return segmentShotUpdates;
};
